...
| **Concurrency primitives** | lock-free SPSC ring buffer + Vyukov-style MPSC intrusive linked list; CAS state machines; `ThreadPool` (`packaged_task`/`future`, 2 workers, one shared queue); `std::shared_mutex` SWMR for `Leaderboard` and `WeatherSystem` |
