...
TelemetryFrame TelemetryGenerator::generateFrame(uint32_t i) {
    ...
    float base_treshold = 0.65f + (driver.tire_management * 0.25f);
    float risk_adjustment = (driver.risk_tolerance - 0.5f) * 0.15f;
    float pit_treshold = base_treshold + risk_adjustment;

    if (state.tire_wear > pit_treshold && !state.is_on_pit) {
    ...
    float speed = 0.0f;
    if (!state.is_on_pit) {
        float driver_skill = 0.80f + driver.consistency * 0.025f;
        speed = 220.0f * car.engine_power * driver_skill * (1.0f - state.tire_wear * 0.4f);
    }
    if (!state.is_on_pit){
        state.tire_wear += 0.005f * track_.tire_wear_factor * driver.agression;
